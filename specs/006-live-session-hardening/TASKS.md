...
- [ ] 1. Issue 1: treat today’s still-open daily row as incomplete. Do not run `multi_day_move`, `drawdown_from_high`, or `relative_to_spy` on it. Prove an unfinished today row that would cross a daily threshold does not emit or save daily “already emitted” state.
- [ ] 2. Issue 1: prove a second backfill in the same open session with a moved still-moving close does not create or escalate a daily event (TSLA vs `SPY` shape). Prove that after the clock is closed, the finished today daily bar may emit.
- [ ] 3. Issue 2: add `session_gap` / `SESSION_OPEN` (3% / 5% / 8%, rearm 1.5%, no volume dampen). Unit-test +6.67% → HIGH UP, +2% quiet, −3% → MODERATE DOWN, Friday close → Monday open.
- [ ] 4. Issue 2: run the gap check once per symbol per session after backfill and/or on the first regular-session minute. Prove a 10:25 start still fires from the stored 09:30 open, and a same-session restart does not research again. Wire it on the live path only; offline fixture mode stays as it is unless a tiny fixture is needed for the unit tests.
